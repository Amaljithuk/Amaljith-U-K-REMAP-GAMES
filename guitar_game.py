...
class RhythmHeroAR:
    ...
    def create_gameover_ui(self):
        ...
        self.gameover_buttons = [self.play_again_btn, self.menu_exit_btn]
    ...
    def _handle_clicks(self, mx, my):
        if self.state == GameState.HOME:
            ...
            if self.hud.check_quit_clicked((mx,my), True):
                self.state = GameState.GAME_OVER
        elif self.state == GameState.GAME_OVER:
            if self.play_again_btn.is_clicked((mx,my), True):
                self.init_game()
                self.state = GameState.CALIBRATION
            if self.menu_exit_btn.is_clicked((mx,my), True):
                self.state = GameState.HOME
    ...
    def update(self, dt):
        ...
        if self.state in (GameState.HOME, GameState.SETTINGS, GameState.INSTRUCTIONS, GameState.CALIBRATION, GameState.GAME_OVER):
            if self.state == GameState.HOME:
